...
`.github/workflows/deploy.yml` handles every deploy after that: on push to
`main`, it runs the full test suite (`ci.yml`), then SSHes into the Droplet,
pulls, rebuilds, and restarts the service — only if tests passed.
